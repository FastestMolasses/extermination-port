...
|---|---|---|---|
| #0–5, #6 | pickups 00219550 ×6, 0015AFA0 | one group adapter on #0: legacy `em_pickup_update` (since WP-5 its battery take posts 001C47A0's B0/B1; the Found line is deleted); the other nodes no-op, marked `group:#0`. The census asserts #0–6 are contiguous. | WP-6: `em_pickup_original_tick` per uid; take writes B0/B1 |
...
| Worker | Binding |
|---|---|
| w_0015BCF0 | `em_player_0015BCF0`: actor_context/actor_update/pose finish, damage/vitals, and the B9 write per 0015CF90. In cutscenes it is bound to the current opening-player path while the opening runtime owns the player (until WP-9/WP-15), and to `em_player_0015BCF0` otherwise (WP-4: the interaction host's shared player worker runs there, through the stage hook). |
...
| w_001AAD00 | `em_interaction_scene_publish` plus the class-list swap and the nine close-out hooks. Since WP-4 the interactive-list swap is the AREA11 interaction host's publication (`em_area11_interaction_host_publish`); the rest stays unmirrored. |
