...
- **Status screen:** the legacy `em_hud`, with invented "CONTENT TBD"/"PARTIAL" pages. Music keeps playing and the menu sounds are silent.
...
slot0 startup_task: logos → E900 → title → New Game → begin_movie(0,1) → movie_pump → em_game_install_new (em_game.c:5803)
    → game_load_task (:5579): player EMDL, scene_manifest_load(assets/scene_snow/scene.txt), collision, em_sfx_init, manifest BGM
    ...
    → game_task → ingame_frame_machine (:5219): case 0 (init, falls through the same tick) → case 1 selects:
       ...
       gameplay_frame (:4679): [hud-open / game-over early-outs] → truck → director_tick (kCineBeats) → grate
...
| H21 (INV-02) | MISSING | `src/game/em_game.c:5529` (level-exit arms "pending") | There are two AREA11 exits: fan 001B0C60(1,1,4), and Roger departure 0x828A10 → 001B0C60(1,0,4). The area-change request is D_008106B5..B8 → 001AD010 → sub-state 5 → frame case 0. | Implement the area-change consumer (ORCH-06) and targets for AREA01 sub 1 (not exported) and sub 0 (scene_drawbridge) at entry 4 (WP-11). |
| H22 (AM-06) | NOT_WIRED (severity corrected to medium, kept here because it is reached on every status open) | `src/game/em_hud.c:1261` | anim_frame_top_b state 1, r==2: 001FBC50 stop-all SFX, 001FABB0 stop streams, 00119828 ×2. On exit, 001FAE70(1) restarts cue 25 with a 270+rand fade. | Comes free with WP-5. `em_status_frame` emits these. Update the stale note at `em_sfx.c:477`. |
...
- **Scope:**
  ...
  - Move the em_hud and game-over early-outs out of `gameplay_frame`.
...
- **Scope:**
  - Replace `em_hud_update/render` with `em_status_runtime`.
...
- **Scope:**
  ...
  - Port the area-change consumer: D_008106B5..B8 → 001AD010 → 001ADF50 load wait → frame case 0 → 001AFCA0 → 001B07C0 spawn placement (ORCH-06).
...
- **Scope:**
  ...
  - New Game movie skip on START only (SI-09).
  - Black plus the 001ADF50 loading shimmer instead of fade-in 4 (SI-10).
