...
- **Status (2026-09-24): LIVE for route 14 (census L22, FIRST_LEVEL_CENSUS.md section 1.10).** `em_area11_roger` binds 008237E0 / em_roger_tick and the equipment 001C5C90 over their original record bytes; the four programs run on `em_area11_script_host`; 0022EEF0 / 0022EC30 (em_cinematic_playback, now in COMMON) drive the bank 0x96 camera in top mode 3; the player's takeover runs 00183090 / 00182DF0 on the record (the special bank 0x96); the encounter stream is cue 29 on the lane-0 stand-in and 001FAE70(0) resumes the music; Roger and the equipment draw through the rig path. The level smoke's `roger` phase equals route 14 f288..f1818 row for row. Open: beat 10's alternate 0x828990 (needs WP-10), the voiced line (WP-8b), the armed talk 0x828810 (bound, no capture on the route), the departure 0x828A10 (not in the first visit; its op0F handshake is a fail-stop NULL), Roger's drop shadow (001DA6A0 reported). `em_roger_runtime` / `em_roger_assets` and `test_roger_encounter_reference` are deleted.
- **Scope:**
...
- **Status (2026-09-25): BINDING PREPARED, BLOCKED ON WP-8b.** The adapter `tick_director_original` (em_area11_bindings.c) binds 008253F0 over em_area11_script_host (op0D sub 2's 0018CBD0 / 0018D7B0 now bound in the host; 001B1EA0 over the one SDK atan2f); in the level smoke's director verification run (`make test-level-smoke-director`) it equals route 10 f1090..f1162 row for row (LEVEL_SMOKE.md "cage_roof prefix"). At f1163 Roger's 0x828990 presents the voiced line 0x7F and the message service faults at 001FA5A0 (voice_push; the voice lanes 001F9CF0 are not live); the director's lines 0x97 / 0x99 are voiced as well. Bound live it would stop the level where the stand-in completes the beat, so node #21 stays on `em_director.c` until WP-8b (DIRECTOR_ORIGINAL.md section 6).
