...
{Host *h=context;h->acquires++;return h->acquire_result;}
static int idle(void *context,float *palette)
{Host *h=context;h->idles++;palette[0]=-42;return 1;}
static int publish(void *context,const float *palette)
{Host *h=context;h->published++;h->last_pose=palette[0];h->last_event=1;return 1;}
static int release(void *context)
{Host *h=context;assert(h->last_event==1);h->releases++;h->last_event=2;return h->release_result;}
static int frame_event(void *context,EmInteractionFrameEvent event)
{Host *h=context;(void)event;h->events++;return 1;}
static int camera(void *context)
{Host *h=context;h->cameras++;return 1;}
static void word(unsigned char *record,unsigned offset,unsigned value)
{for (unsigned i=0;i<4;i++)record[offset+i]=(unsigned char)(value>>(8*i));}

int main(void)
{
    EmModelClip clips[]={{0x47,0,200,60},{0x15C,200,121,60}};
    EmModel model={.bone_count=22,.frame_count=321,.clip_count=2,.clips=clips};
    model.palette=calloc(321*22*16,sizeof(float));assert(model.palette);
    for (unsigned f=0;f<321;f++)model.palette[f*22*16]=(float)f;
    float local[22*16]={0};
    ...
    unsigned char record[64]={0};word(record,0,7);word(record,8,2);
    EmScript script={0};
    ...
    for (int i=0;i<150;i++)assert(em_interaction_runtime_player_tick(&runtime,0)==0);
    ...
    for (int i=0;i<8;i++)assert(em_interaction_runtime_player_tick(&runtime,1)==1);
    ...
    frame.selector=0;host.release_result=0;
    ...
    memset(&frame,0,sizeof frame);hooks.acquire_player=NULL;
    assert(em_interaction_runtime_init(&runtime,&frame,&model,local,&hooks));
    assert(em_interaction_runtime_claim(&runtime,&panel));
    assert(em_interaction_runtime_player_tick(&runtime,1)==-1 && !frame.player_ready);
    free(model.palette);
    puts("Shared interaction ownership, real readiness boundary, status freeze, palette order and release faults: PASS");
    return 0;
}
