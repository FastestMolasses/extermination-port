...
int em_model_load(EmModel *m, const char *path)
{
    ...
    if (fread(&hdr, sizeof(hdr), 1, f) != 1 ||
        memcmp(hdr.magic, "EMD1", 4) != 0) {
        fprintf(stderr, "emdl: %s is not an EMD1 file\n", path);
        fclose(f);
        return 1;
    }
