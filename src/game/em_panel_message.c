...
int em_panel_message_load(EmPanelMessage *message, const char *path)
{
    ...
    if (fread(header, 1, 32, f) != 32 || memcmp(header, "EMOD", 4) || u32(header + 4) != 1 ||
        u32(header + 8) != 2 || !u32(header + 12) || u32(header + 12) > 64 ||
        u32(header + 24) > 448 || !u32(header + 28) || u32(header + 28) > 2048 ||
        fread(records, 1, 40, f) != 40) {
        fclose(f);
        return 0;
    }
    ...
    for (unsigned i = 0; valid && i < 2; i++) {
        ...
        if (valid)
            message->lines[i] = (EmOpeningLine){.line = (uint16_t)u16(r),
                                                .duration = (uint16_t)u16(r + 2),
                                                .voice = -1,
                                                .speaker = 255,
                                                .terminal = r[7],
                                                .skew = r[16],
    ...
    message->text = text;
    message->line_height = u32(header + 12);
    message->fill = u32(header + 16);
    message->outline = u32(header + 20);
    message->y = u32(header + 24);
...
int em_panel_message_tick(EmPanelMessage *message, int busy155, int busy156)
{
    ...
    if (message->dialogue.active)
        em_opening_dialogue_tick(&message->dialogue);
    if (!message->dialogue.active && !busy155 && !busy156)
        message->phase = 2;
    return 0;
}
