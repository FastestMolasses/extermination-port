...
{
    if (!runtime || !frame || !model || !model->bone_count ||
        !local_palette || !hooks) return 0;
...
{
    if (!runtime || !owner || runtime->failed || !runtime->frame ||
        runtime->owner || runtime->frame->selector || runtime->frame->ready ||
        runtime->frame->player_ready) return 0;
    runtime->owner=owner;
    ...
    const void *owner,EmScript *script,const unsigned char *record)
{
    if (!em_interaction_runtime_owns(runtime,owner) || !script || !record ||
        (em_script_u32(record,0)&0xFFF)!=7) return EM_SCRIPT_UNSUPPORTED;
    EmScriptCommandResult result=em_interaction_frame_command(runtime->frame,
        script,em_script_u32(record,8),em_script_u32(record,0x14)!=0,
        runtime->hooks.frame_event,runtime->hooks.context);
    if (result==EM_SCRIPT_UNSUPPORTED) fault(runtime);
    return result;
}

int em_interaction_runtime_camera_retarget(EmInteractionRuntime *runtime,const void *owner)
{
    if (!em_interaction_runtime_owns(runtime,owner)) return 0;
    if (!runtime->frame->player_ready || !runtime->hooks.camera_retarget ||
        runtime->hooks.camera_retarget(runtime->hooks.context)!=1) {
        fault(runtime);return 0;
    }
    return 1;
}
    ...
    const void *owner,uint16_t clip,float rate,float blend)
{
    ...
    if (!runtime->frame->player_ready ||
        !em_interaction_animation_request(&runtime->animation,runtime->model,
                                          clip,rate,blend)) {
        fault(runtime);return 0;
    }
    return 1;
...
int em_interaction_runtime_player_tick(EmInteractionRuntime *runtime,int ordinary_tasks_enabled)
{
    ...
    if (frame->player_ready!=1) return fault(runtime);
    int palette_result;
    if (runtime->animation.active)
        palette_result=em_interaction_animation_tick(&runtime->animation,
                                  runtime->model,runtime->local_palette);
    else if (hooks->idle_player_tick)
        palette_result=hooks->idle_player_tick(hooks->context,runtime->local_palette);
    else return fault(runtime);
    if (palette_result<0 || palette_result>1) return fault(runtime);
