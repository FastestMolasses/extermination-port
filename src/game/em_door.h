 ...
 * the only shipped links are intra-area sub-state moves; BGM and the
 * sfx registry persist). <scene-dir> with no '/' is the SIBLING
 * directory name of the current scene dir. NOTE the AREA02 decode
 * found NO real door pair linking the two exported office sub-states
 * (each one's real dests are other, unexported areas) — the shipped
 * links are SYNTHETIC, flagged in the manifest comments, wiring the
 * two scenes' nearest doors to prove the machinery. EM_DOOR_TEST
 * ignores goto tails (it asserts the same-scene re-place geometry).
 *
