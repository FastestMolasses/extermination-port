...
typedef struct { uint32_t u,v,w,h; } Sprite;
typedef struct { char *string; uint8_t *spans; uint32_t count; } Text;
struct EmBatteryUI {
    uint8_t *data, *pixels;
...
EmBatteryUI *em_battery_ui_load(const char *path)
{
    ...
    if (fread(header,1,32,f)!=32 || memcmp(header,"EMBA",4) ||
        u32(header+4)!=1 || u32(header+16)!=28 || u32(header+20)!=8) {
        fclose(f);return NULL;
    }
    uint32_t w=u32(header+8),h=u32(header+12),texts=u32(header+24),size=u32(header+28);
    if (!w || !h || w>4096 || h>4096 || texts>16384 ||
        (uint64_t)w*h*4+28*32+texts+96!=size) {fclose(f);return NULL;}
    EmBatteryUI *ui=calloc(1,sizeof *ui);
    if (!ui) {fclose(f);return NULL;}
    ui->data=malloc(size);
    if (!ui->data || fread(ui->data,1,size,f)!=size || fgetc(f)!=EOF) {
        fclose(f);em_battery_ui_free(ui);return NULL;
    }
    fclose(f);ui->width=w;ui->height=h;
    uint8_t *p=ui->data;
    for (unsigned i=0;i<28;i++,p+=32) {
        Sprite *s=&ui->sprites[i];
        s->u=u32(p+4);s->v=u32(p+8);s->w=u32(p+12);s->h=u32(p+16);
        if (u32(p)!=i || !s->w || !s->h || s->w>w || s->h>h ||
            s->u>w-s->w || s->v>h-s->h) goto invalid;
    }
    uint8_t *end=p+texts;
    for (unsigned i=0;i<8;i++) {
        if (end-p<8) goto invalid;
        uint32_t length=u32(p),count=u32(p+4);p+=8;
        if (!length || length>1024 || count>32 || (uint64_t)length+count*8>(uint64_t)(end-p)) goto invalid;
        ui->text[i].spans=p;ui->text[i].count=count;p+=count*8;
        ui->text[i].string=(char *)p;
        if (p[length-1] || memchr(p,0,length-1)) goto invalid;
        for (uint32_t j=0;j<count;j++) {
            uint32_t at=u32(ui->text[i].spans+j*8);
            if (at>=length || (j && at<u32(ui->text[i].spans+(j-1)*8))) goto invalid;
        }
        p+=length;
    }
    ...
    return ui;
invalid:
    em_battery_ui_free(ui);return NULL;
}

void em_battery_ui_free(EmBatteryUI *ui)
{
    if (!ui) return;
    em_battery_ui_close(ui);free(ui->data);free(ui);
}

int em_battery_ui_begin(EmBatteryUI *ui, EmPanel *owner, int charge, int kind)
{
    if (!ui || !owner || kind<0 || kind>2 || charge<0 || charge>255) return 0;
    ui->owner=owner;ui->kind=kind;ui->active=1;ui->uploaded=0;
    ui->error_timer=ui->draw_error=0;
    em_panel_battery_begin(&ui->menu,charge);
    ui->draw_charge=charge;ui->draw_phase=ui->menu.phase;
    return 1;
}
...
unsigned em_battery_ui_tick(EmBatteryUI *ui, unsigned buttons, int *charge,
                             int owner_available)
{
    if (!ui || !ui->active || !charge) return 0;
    ui->draw_phase=ui->menu.phase;ui->draw_charge=*charge;
    ui->draw_error=ui->error_timer!=0;
    if (ui->error_timer) {
        if (buttons&0x60) {ui->error_timer=0;return EM_PANEL_MENU_CANCEL;}
        --ui->error_timer;return 0;
    }
    if (ui->menu.phase==EM_PANEL_MENU_BROWSE) {
        if (buttons&0x20) return EM_BATTERY_BACK_TO_STATUS|EM_PANEL_MENU_CANCEL;
        if (buttons&0x40) {
            /*00185420 may no longer find an eligible associated device. */
            if (!owner_available) {ui->error_timer=240;return EM_BATTERY_NO_DEVICE_SOUND;}
            em_panel_battery_begin(&ui->menu,*charge);
...
static void sprite(EmBatteryUI *ui, EmGfx *gfx, int id, float x,float y,
                    float w,float h,uint32_t rgba)
{
...
static void gs_sprite(EmBatteryUI *ui, EmGfx *gfx, int id, int x,int y,
                       int w,int h,uint32_t rgba)
{
...
{
    Text *t=&ui->text[index];uint32_t span=0,color=0x606060;
    float pen=x;
    for (unsigned i=0;t->string[i];i++) {
        while (span<t->count && u32(t->spans+span*8)==i) {
            color=u32(t->spans+span*8+4);span++;
        }
        char c=t->string[i];
        if (c=='\n') {pen=x;y+=24;continue;}
        char glyph[2]={c,0};
...
{
    char caption[16];snprintf(caption,sizeof caption,"%02d/%02d",ui->draw_charge>>1,capacity>>1);
    ...
    for (int row=0;row<4;row++) {
        ...
        float delta[4];
        for (int c=0;c<4;c++) delta[c]=em_effect_float32((double)(goal[c]-color[c])/12.0);
        for (int column=0;column<12 && row*12+column<ui->draw_charge;column++) {
            uint32_t rgba=0;
            for (int c=0;c<4;c++) rgba|=(uint32_t)(unsigned)color[c]<<(c*8);
            sprite(ui,gfx,14,320-column*12-(!(column&1)),202+row*12,12,12,rgba);
            for (int c=0;c<4;c++) color[c]=em_effect_float32((double)color[c]+delta[c]);
        }
    }
}
...
{
    if (!ui || !ui->active || !gfx) return 0;
    if (!ui->uploaded) {
        if (!em_gfx_overlay_texture_set(gfx,EM_GFX_OVERLAY_TEX_UI,
                                          ui->pixels,ui->width,ui->height)) return 0;
        em_hud_decor_invalidate();ui->uploaded=1;
    }
    em_gfx_overlay_canvas(gfx,512,448);
    Sprite *bg=&ui->sprites[26];em_hud_background_sprite(gfx,bg->u,bg->v,bg->w,bg->h);
    ...
    if (ui->draw_error) text(ui,gfx,7,138,336);
    else if (ui->draw_phase==EM_PANEL_MENU_CONFIRM) {
        text(ui,gfx,1,138,336);text(ui,gfx,0,270,408);
        sprite(ui,gfx,27,ui->menu.no_selected?335:253,412,12,12,0x80CE6000);
    } else if (ui->draw_phase==EM_PANEL_MENU_INSUFFICIENT) text(ui,gfx,2,138,336);
    else if (ui->draw_phase==EM_PANEL_MENU_BROWSE) text(ui,gfx,3+ui->kind,138,336);
...
{
    if (!ui) return;
    if (ui->uploaded) em_hud_decor_invalidate();
    ui->active=ui->uploaded=0;ui->owner=NULL;
}
